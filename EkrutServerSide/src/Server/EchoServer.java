...
public class EchoServer extends AbstractServer 
{
	...
	*/
	...
	private void InitiliazeQuerys()
	{
		...
		SqlQuerys.put("#UPDATE_CLIENT_TO_BE_SUBCRICBER", "table=registrationformtable#values="
				+ "firstName=@"
				+ "&lastName=@"
				+ "&telephone=@"
				+ "&Email=@"
				...
				+ "&userPassword=@"
				+ "&area=@"
				+ "&userTypeRequest=Registered To Subscriber"
				+ "&isSentToManager=0"
				);
		...
		SqlQuerys.put("#GET_ALL_SALES_CLIENT_VIEWER", "table=sales#values=saleType=saleType&Item=Item#condition=area=@&isActive=1");
		...
		SqlQuerys.put("#REFILL_PUT_TO_EXEC_ORDER", "table=executiveorders#values=ProductName=@&"
				+ "ProductQuantity=@&Area=@&FacilityLocation=@&"
				+ "FacilityName=@&FacilityThresholdLevel=@");

		SqlQuerys.put("#REFILL_GET_THRESHOLD", "table=facilities#condition=FacilityLocation=@&FacilityName=@");

		SqlQuerys.put("#REFILL_GET_PRODUCTS", "SELECT products.ProductName, productsinfacility.ProductAmount, facilities.FacilityName, facilities.FacilityThreshholdLevel,facilities.FacilityLocation,facilities.FacilityID "
		...
		SqlQuerys.put("#THRESHOLD_UPDATE_FACILITY", "table=facilities#condition=FacilityID=@#values=FacilityThreshholdLevel=@");
		...
		SqlQuerys.put("#GET_CUTOMER_HISTOGRAM",
				"SELECT users.userName,COUNT(*) AS 'Number of Purchases' "
